...
svmcm <- confusionMatrix(svmpred, data$hdi_cat)
svmcm$table
```
...
An MLP is a type of feedforward artificial neural network. This is a very simple NN, 
...
mlpcm <- confusionMatrix(mlppred, data$hdi_cat)
mlpcm
```
...
The tree based models I've chosen are *eXtreme Gradient Boosting* and *Boosted Logistic Regression*. Both yield outstandingly good results and we could suspect both of these were, perhaps, overfitted to the data, however, this does not prevent us from making a final conclusion about how our variables relate to HDI.
...
xgbtreecm <- confusionMatrix(xgbtreepred, data$hdi_cat)
xgbtreecm$table
```
...
logitboostcm <- confusionMatrix(logitboostpred, data$hdi_cat)
logitboostcm$table
```
